...
use App\Authorization\AuthDecodeHelper;
...
class BooklistController
{
    ...
    public function createBooklist(Request $request, Response $response)
    {
        ...
        // 在关系表内添加条目
        $uid = AuthDecodeHelper::getUserIdFromRequest($request);
    ...
    {
        $uid = $args['id'];
        $createBooklists = WechatUser::find($uid)
            ->booklists()
            // 找到其中该用户创建的书单
            ...
            ->get();
        $favoriteBooklists = WechatUser::find($uid)
            ->booklists()
            // 找到其中不是该用户创建的书单
            ->whereHas('creator', function ($query) use ($uid) {
                $query->where('id', '!=', $uid);
