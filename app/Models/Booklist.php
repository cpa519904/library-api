...
class Booklist extends Model
{
    ...
    {
        return $this->belongsToMany('App\Models\SimpleBook', 'book_booklist', 'booklist_id', 'book_id')
            ->withPivot('comment')
            ->withTimestamps(); // sync 时自动更新中间表的 updated_at 字段，当书单内已有的图书再次被加入书单时，会显示在前面
    }
    ...
    public function users()
    {
        return $this->belongsToMany('App\Models\WechatUser', 'booklist_wechat_user', 'booklist_id', 'wechat_user_id')
            ->withTimestamps(); // sync 时自动更新中间表的 updated_at 字段，当用户重新收藏书单时，会显示在前面
    }
    ...
    {
        $books = $this->books()
            ->orderBy('book_booklist.updated_at', 'desc')
            ->offset($offset)
            ->limit($limit)
            ->get();
